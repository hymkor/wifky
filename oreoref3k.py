...
class MkdirLockError(Exception):
    pass

class IronShelve(dict):
    ...
    def commit(self):
        if not self._modify :
            logging.debug("data not modified")
            return
        ...
        ### Write file ###
        try:
            f = open(self._filename+".p","wb")
            pickle.dump( self , f )
            f.close()
            logging.debug("commit done on {0}".format(self._filename))
        finally:
            ### Release Lock ###
            os.rmdir(lockfn)
...
def del_cookie(key):
    print('Set-Cookie: %s=; expires=Fri, 31-Dec-1999 23:59:59 GMT; path=%s' \
        % ( urllib.parse.quote_plus(key) , os.getenv("SCRIPT_NAME","") ))
    ...
    def __init__(self,form):
        ...
        if self.user == None :
            ...
            for key,val in data.items():
                logging.debug("UserAplication.__init__:{0}=>{1}".format(key,val))
