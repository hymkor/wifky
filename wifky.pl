...
sub init_globals{
    ...
    %::preferences = (
        ' General Options' => [
            { desc=>'script-revision '.$::version.' $Date: 2007/11/17 01:18:55 $' ,
              type=>'rem' },
...
sub init_menubar{
    if( &is_signed() ){
        ...
        delete $::menubar{'900_SignOut'};
        delete $::menubar{'500_Tools'};
    }
...
sub action_signin{
    ...
    &putenc(qq(<form action="%s" method="POST" accept-charset="%s">
        ...
        <input type="hidden" name="a" value="signinext">
        <input type="submit" value="Enter">)
        , $::postme , $::charset );

    while( my ($key,$val)=each %::form ){
        if( $key =~ /_t$/ ){
            &putenc('<input type="hidden" name="_%s_y" value="%s" />' ,
                        $` , &yen($val) );
        }elsif( $key ne 'a'  ||  $val ne 'signin' ){
            &putenc('<input type="hidden" name="_%s" value="%s" />', $key , $val );
        }
    }
    &puts('</p></form>');
    &print_footer;
